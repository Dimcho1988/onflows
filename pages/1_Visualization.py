...
def fetch_last30_segments_for_fit(user_id: int, sport_group: str, segment_seconds: int, model_key: str, n_acts: int = 30) -> pd.DataFrame:
    ...
    # fetch segments for those ids (same model_key & segment_seconds)
    res2 = (
        supabase.table("activity_segments")
...
st.title("onFlows – Visualization (with last-30 regression curve)")
...
with tabs[1]:
    ...
    if show_last30_curve:
        ...
        if poly is None:
            ...
            st.caption(f"Last-30 fit: poly deg=2 (shifted to F(0)=1), then clipped + alpha={alpha_slope_viz:.2f}")
